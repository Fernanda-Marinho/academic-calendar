...
function buildICS(events, calName) {
  ...
  events.forEach((ev, idx) => {
    const startDateStr = `${ev.y}${pad(ev.m)}${pad(ev.d)}`;
    const endDateStr = `${ev.endY}${pad(ev.endM)}${pad(ev.endD)}`;
    const uid = `${startDateStr}-${idx}-${Math.random().toString(36).slice(2)}@uefs-calendar-ics`;

    lines.push("BEGIN:VEVENT");
    lines.push(`UID:${uid}`);
    lines.push(`DTSTAMP:${dtstamp}`);
    lines.push(`DTSTART;TZID=America/Bahia:${startDateStr}T090000`);
    lines.push(`DTEND;TZID=America/Bahia:${endDateStr}T100000`);
  ...
  async function handleCalendarClick(cal) {
    ...
    try {
      ...
      if (!data.events || data.events.length === 0) {
        throw new Error(
          "Nenhum evento foi encontrado nesse PDF. O formato pode ser diferente do esperado :(",
        );
      }
      ...
      setStatus(
        `${data.events.length} evento(s) encontrados. Baixe o arquivo e importe no Google Agenda.`,
      );
      setStep("result");
    } catch (err) {
  ...
  return (
    <div className="app">
      <header>
        ...
        <p>
          Escolha um calendário, baixe o arquivo <code>.ics</code> e importe no
          Google Agenda.
        </p>
      </header>
